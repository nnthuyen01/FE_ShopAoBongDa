    ...
    if (loading) {
        ...
        if (order.status !== "Đã thanh toán") {
            setButtonChangeStatus = (
                <button onClick={() => changeStatus(order.id)} className='btn btn-success btn-sm'>Update order status</button>
            )
        }
                            ...
                            <div className="card" style={{ borderRadius: "10px" }}>
                                ...
                                <div className="card-body p-4">
                                    <div className="d-flex justify-content-between align-items-center mb-4">
                                        <p className="lead fw-normal mb-0" style={{ color: '#a8729a' }}>{order.status === 0 ? 'Unconfirm' : order.status === 1 ? 'Unpaid' : 'Paid'}</p>
                                        {setButtonChangeStatus}
                                    </div>
                                    <div className="d-flex justify-content-between pt-2 mb-4">
                                        <p className="fw-bold mb-0">Receiver: <span style={{ color: "#6c757d" }}>{order.name}</span></p>
                                        <p className="fw-bold mb-0">Phone: <span style={{ color: "#6c757d" }}>{order.phone}</span></p>
                                        <p className="fw-bold mb-0">Account: <span style={{ color: "#6c757d" }}>{order.username}</span></p>
                                    </div>
                                    <div className="d-flex justify-content-between pt-2 mb-4">
                                        <p className="fw-bold mb-0">Address: <span style={{ color: "#6c757d" }}>{order.address}</span></p>
                                    </div>
                                    {display_Productdata}
                                    <div className="d-flex justify-content-between pt-2">
                                        <p className="fw-bold mb-0">Invoice information</p>
                                        <p className="text-muted mb-0"><span className="fw-bold me-4">Total amount:</span>{numberFormat(order.totalPriceOrigin)}</p>
                                    </div>

                                    <div className="d-flex justify-content-between pt-2">
                                        <p className="text-muted mb-0">ID: {order.id}</p>
                                        <p className="text-muted mb-0"><span className="fw-bold me-4">Discount:</span>{numberFormat(order.priceOff)}</p>
                                    </div>

                                    <div className="d-flex justify-content-between">
                                        <p className="text-muted mb-0">Date: {formatDate(order.date)}</p>
                                        <p className="text-muted mb-0"><span className="fw-bold me-4">Price ship:</span>{numberFormat(order.priceShip)}</p>
                                    </div>
                                </div>
                                <div className="card-footer border-0 px-4 py-5"
                                    ...
                                    <h5 className="d-flex align-items-center justify-content-end text-white text-uppercase mb-0">
                                        Total payment: <span className="h2 mb-0 ms-2">{numberFormat(order.totalPrice)}</span></h5>
                                </div>
                            </div>
                        </div>
                    </div>
